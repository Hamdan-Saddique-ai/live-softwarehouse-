...
const COURSES = [
  {img:'assets/courses/1.png',  title:'AI & Chatbots',        dur:'8 Weeks',  level:'Intermediate', offer:true},
  {img:'assets/courses/2.png',  title:'Python Programming',   dur:'10 Weeks', level:'Beginner',     offer:false},
  {img:'assets/courses/3.png',  title:'Web Development',      dur:'12 Weeks', level:'Beginner',     offer:true},
  {img:'assets/courses/4.png',  title:'App Development',      dur:'12 Weeks', level:'Intermediate', offer:false},
  {img:'assets/courses/5.png',  title:'WordPress',            dur:'6 Weeks',  level:'Beginner',     offer:false},
  {img:'assets/courses/6.png',  title:'Shopify',              dur:'6 Weeks',  level:'Beginner',     offer:true},
  {img:'assets/courses/7.png',  title:'Graphic Design',       dur:'8 Weeks',  level:'Beginner',     offer:false},
  {img:'assets/courses/8.png',  title:'Video Editing',        dur:'6 Weeks',  level:'Beginner',     offer:false},
  {img:'assets/courses/9.png',  title:'YouTube Automation',   dur:'5 Weeks',  level:'Beginner',     offer:true},
  {img:'assets/courses/10.png', title:'TikTok Automation',    dur:'5 Weeks',  level:'Beginner',     offer:false},
  {img:'assets/courses/11.png', title:'Facebook Monetization',dur:'5 Weeks',  level:'Beginner',     offer:false},
  {img:'assets/courses/12.png', title:'Instagram Automation', dur:'5 Weeks',  level:'Beginner',     offer:true},
  {img:'assets/courses/13.png', title:'Digital Marketing',    dur:'9 Weeks',  level:'Intermediate', offer:false},
  {img:'assets/courses/14.png', title:'Cyber Security',       dur:'10 Weeks', level:'Advanced',     offer:false},
];
...
function drawFrameCover(ctx, canvas, img){
  ...
  let dw, dh, dx, dy;
  /* on a normal / landscape screen this fills edge-to-edge like a background;
     but on a tall narrow phone (cr very small vs the frame's own ratio) a
     strict "cover" would crop away a huge chunk of the frame's left/right
     content. Past a safe crop limit we blend toward "contain" instead, so
     every device keeps the full frame visible and just adjusts how much
     letterboxing it gets — no device-specific breakpoints needed. */
  const cropRatio = ir > cr ? (cr/ir) : (ir/cr); // how much of the frame a strict cover would keep, 0..1
  const MIN_KEEP = 0.38; // only phones (very tall/narrow) fall below this — tablets/laptops keep the original full-cover look
  if(cropRatio < MIN_KEEP){
    const t = cropRatio / MIN_KEEP; // 0..1, how close we are to the safe limit
    if(ir > cr){
      const coverW = ch*ir, containW = cw;
      dw = containW + (coverW-containW)*t; dh = dw/ir; dx=(cw-dw)/2; dy=(ch-dh)/2;
    } else {
      const coverH = cw/ir, containH = ch;
      dh = containH + (coverH-containH)*t; dw = dh*ir; dy=(ch-dh)/2; dx=(cw-dw)/2;
    }
...
/* ---------------- ENROLL MODAL — collects details, sends them to WhatsApp ---------------- */
const WHATSAPP_NUMBER = '923327330518'; // 03327330518 in international format, no leading 0/+
let enrollSelectedCourse = '';

function openEnrollModal(courseTitle){
  enrollSelectedCourse = courseTitle || '';
  const overlay = document.getElementById('enrollOverlay');
  const nameEl = document.getElementById('enrollCourseName');
  if(nameEl) nameEl.textContent = enrollSelectedCourse || 'enrollment';
  if(!overlay) return;
...
function initEnrollModal(){
  ...
  form.addEventListener('submit', e=>{
    ...
    let valid = true;
    const checks = [
      {id:'en-name',  ok: v=> v.trim().length>1},
      {id:'en-age',   ok: v=> v.trim().length>0 && Number(v)>0 && Number(v)<100},
      {id:'en-phone', ok: v=> v.replace(/\D/g,'').length>=10},
      {id:'en-city',  ok: v=> v.trim().length>1},
    ];
    checks.forEach(c=>{
      const input = document.getElementById(c.id);
      const field = input.closest('.field');
    ...
    const city  = document.getElementById('en-city').value.trim();
    const notes = document.getElementById('en-notes').value.trim();

    const lines = [
      'New Course Enrollment',
      `Course: ${enrollSelectedCourse || '-'}`,
      `Name: ${name}`,
      `Age: ${age}`,
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  safe(initEnrollModal, 'enrollModal');
